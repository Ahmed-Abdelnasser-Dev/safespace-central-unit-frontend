...
/**
 * Header for all screens with modern styling
 */
      ...
      <div className="flex items-start justify-between gap-6">
        {(title || description) && (
          <div className="flex-1 animate-slideUp">
            {title && (
              <h1 className="font-display text-3xl font-bold text-safe-text-dark">{title}</h1>
            )}
            {description && (
              <p className="text-sm text-safe-text-gray/80 mt-2 font-light">{description}</p>
            )}
          </div>
        )}
