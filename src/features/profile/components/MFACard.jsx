...
function StatusBadge({ enabled }) {
  return (
    <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full ${
      enabled
        ? 'bg-safe-success/10 text-safe-success border border-safe-success/20'
        : 'bg-safe-bg text-safe-text-muted border border-safe-border'
    }`}>
      <span className={`w-1.5 h-1.5 rounded-full ${enabled ? 'bg-safe-success' : 'bg-safe-text-muted'}`} />
      ...
      <div className="bg-safe-sidebar rounded-xl border border-safe-border p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          {mfaBackupCodes.map((code, i) => (
            <div key={i} className="font-mono text-sm text-center py-1.5 px-3 bg-safe-bg border border-safe-border rounded-lg text-safe-text-dark tracking-wider">
              {code}
            </div>
          ))}
        </div>
      ...
      {mfaEnabled && (
        <div className="space-y-3">
          {!disableOpen ? (
            ...
            <div className="space-y-3 border border-safe-danger/20 rounded-lg p-4 bg-safe-danger/[0.03]">
              <p className="text-xs font-medium text-safe-danger">Confirm to disable 2FA</p>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 py-2 rounded-lg border border-safe-border text-sm text-safe-text-gray hover:bg-safe-bg transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 py-2 rounded-lg bg-safe-danger text-white text-sm font-medium hover:bg-safe-danger/90 disabled:opacity-50 transition-colors"
                >
